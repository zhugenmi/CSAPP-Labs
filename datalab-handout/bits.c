...
int isLessOrEqual(int x, int y) {
  ...
  return (flagx&(!flagy)) | (!(flagx^flagy)&(!flagy_x)); //返回x为负并且y为正 或者 xy同号且y-x为正 的情况
}
//4
/* 
...
int howManyBits(int x) { 
  int signx=x>>31; 
  x=(signx&~x)|(~signx&x); //若x为负，则取~x
  int pos16=!!(x>>16)<<4;   //若高16位中有1，则到高16位中找1
  x=x>>pos16;
  int pos8=!!(x>>8)<<3;     //高8位是否有1
  x=x>>pos8;
  int pos4=!!(x>>4)<<2;
  x=x>>pos4;
  int pos2=!!(x>>2)<<1;
  ...
  int pos1=!!(x>>1);
  x=x>>pos1;
  return pos16+pos8+pos4+pos2+pos1+x+1;
} 
//float
/* 
